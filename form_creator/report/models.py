...
class FormResponse(models.Model):
    form = models.ForeignKey('form_app.Form', on_delete=models.CASCADE, related_name='responses')
...
class ProcessResponse(models.Model):
    process = models.ForeignKey('process_app.Process', on_delete=models.CASCADE, related_name='responses')
    user = models.ForeignKey('auth.User', null=True, blank=True, on_delete=models.SET_NULL)
    completed_forms = models.ManyToManyField(FormResponse, related_name='completed_in_process')
...
class PeriodicReport(models.Model):
    ...
    report = models.ForeignKey(Report, on_delete=models.CASCADE, related_name='periodic_reports')
    frequency = models.CharField(max_length=10, choices=REPORT_FREQUENCY_CHOICES)
